...
sfo_feb_flights %>% group_by(carrier) %>% summarise(arr_delay_IQR= IQR(arr_delay),arr_delay_median= median(arr_delay))
...
nycflights %>% group_by(month) %>% summarise(average_DD=mean(dep_delay)) %>% arrange(desc(average_DD))
...
My dygression for what could be a distribution for flight origin in every month
```{r}
ggplot(data = nycflights, aes(x = factor(month), fill = origin)) + geom_bar()
```
...
nycflights <- nycflights %>% mutate(average_flying_spead=distance/air_time)
nycflights %>% group_by(tailnum) %>% summarise(max_average_flying_spead=max(average_flying_spead)) %>% arrange(desc(max_average_flying_spead))
```
...
ggplot(data = nycflights, aes(x= distance, y= average_flying_spead)) + geom_point()
...
#determine the on time arrival percentage based on whether the flight departed on time or not

nycflights %>% summarise(rate_arr_type = sum(arr_type == "on time")/n())

nycflights %>% filter(dep_type == "delayed") %>% summarise(rate_on_time_while_delayed=sum(arr_type=="on time")/n())
